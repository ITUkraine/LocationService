...
public class LocationController {
    ...
    List<Location> findLocationsWithinProximity(@RequestParam(value = "lat") double latitude, @RequestParam(value = "long") double longitude,
	    @RequestParam(value = "d") double distance, @RequestParam(value = "t1", required = false) Long timeMillis1,
	    @RequestParam(value = "t2", required = false) Long timeMillis2, @RequestParam(value = "s", required = false) List<Long> sIDs) {
	return service.findLocationsWithinProximity(latitude, longitude, distance);
    }
}
